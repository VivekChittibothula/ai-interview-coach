...
- Make the question specific to the role.
...
export default function App() {
  ...
  async function fetchQuestion(idx, sess) {
    ...
    try {
      const previousQuestions = qas.map(
  (item) => item.question
);
...
const q = await askGemini(
  buildQuestionPrompt(
    ...
    previousQuestions
      ...
      setCurrentQuestion("Tell me about yourself and your experience relevant to this role.");
    }
  ...
  async function handleAnswerSubmit(answer) {
    ...
    try {
      ...
      feedback = { score: 5, strengths: ["Attempted the question"], weaknesses: ["Could elaborate more"], improvedAnswer: "A strong answer would include specific examples and clear structure." };
    }
  ...
  async function handleNext() {
    ...
    if (nextIndex >= TOTAL_QUESTIONS) {
      ...
      try {
        ...
        const avg = Math.round(qas.reduce((s, q) => s + (q.feedback?.score ?? 5), 0) / qas.length);
        setReport({ overall: avg, technical: avg, communication: avg - 1, problemSolving: avg, roadmap: ["Practice mock interviews regularly.", "Study data structures and algorithms.", "Work on clear communication.", "Build real-world projects.", "Review common questions for your role."] });
      }
    } else {
